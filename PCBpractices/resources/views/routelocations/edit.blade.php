...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <p class="card-title">Edit Route Locations</p>
                    @if (session('success'))
                                    ...
                                    <div class="card-body">
                                        ...
                                        <table class="table">
                                            ...
                                            <tbody id = "locations_data">
                                                @foreach($editbranches as $editbranche)
                                                    <tr id="{{$editbranche->BranchId}}">
                                                        <td>{{$editbranche->BranchName}} - {{$editbranche->Location}}</td>
                                                        @if(count($editbranches) > 2)
                                                        <td><a href="javascript:delete_editlocation({{$editbranche->BranchId}})"><i class="fa fa-trash-o" style="font-size:35px;color:red"></i>X</a></td>
                                                        @endif
                                                    </tr>
                                                @endforeach
                                            </tbody>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="route_name" id="route_name">
                                                @foreach($routes as $route)
                                                    <option value="{{$route->RouteId}}" {{ $editroute->RouteId == $route->RouteId ? 'selected="selected"' : '' }}>{{ $route->RouteName }}</option>
                                                @endforeach
                                            </select>
                                            ...
                                            <button type="button"  onclick= 'add_route()' class="btn btn-lg btn-success btn-block waves-effect waves-light">UPDATE ROUTE</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    <script>
        ...
        function delete_editlocation(BranchId)
        {
            ...
            $.ajax({
                ...
                success: function(data) {
                    $('.message').html('<div class="alert alert-success">Location Deleted</div>');
        ...
        function add_route(){
            ...
            if(route_name.length === 0){
                ...
                }*/ // editing time can add one location also
                //else{
                    var form_data = {
