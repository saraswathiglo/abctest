...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    <p class="card-title">Add Village</p>
                    @if (session('success'))
                    ...
                    @endif
                    <form method="POST" action="{{ route('village.store') }}" class="form-horizontal">
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control" name="PanchayteeId" required="">
                                ...
                                @foreach($panchaytees as $panchaytee)
                                    <option value="{{ $panchaytee->PanchayteeId }}">{{ $panchaytee->PanchayteeName }}</option>
                                @endforeach
                            </select>
                            @error('PanchayteeId')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="VillageName" value="" class="form-control" placeholder="Enter Village Name" required="">
                            @error('VillageName')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
