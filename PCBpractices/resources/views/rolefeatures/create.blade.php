        ...
        <div class="col-md-12 stretch-card">
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                    <form method="POST" action="{{ route('rolefeatures.store') }}" class="form-horizontal">
                        @csrf
                                ...
                                @foreach($features as $feature)
                                    <div class="col-md-12">
                                        ...
                                        @if($feature->FeatureType == "MainMenu")
                                            <div class="col-md-6">
                                                <label><input type="checkbox" name="OperationId[]" value="{{$role->RoleId}}_{{$feature->FeatureId}}_1"> View</label>
                                            </div>
                                        @endif
                                        ...
                                        @foreach($submenu as $submenurow)
                                            <div class="col-md-3">
                                                ...
                                                @foreach($operations as $operation)
                                                    <div class="col-md-6">
                                                        <label><input type="checkbox" name="OperationId[]" value="{{$role->RoleId}}_{{$submenurow->FeatureId}}_{{$operation->OperationId}}"> {{ $operation->OperationName }}</label>
                                                    </div>
                                                @endforeach
                                            </div>
                                        @endforeach
